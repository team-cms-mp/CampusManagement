...
{
    public partial class ReportViewerExamStudentSubjectClash : System.Web.UI.Page
    {
        ...
        protected void LoadGetEligibleStudentsForExam(string searchOrPrint)
        {
            try
            {
                List<rpt_GetExamStudentSubjectClashReport_Result> StudentList = new List<rpt_GetExamStudentSubjectClashReport_Result>();
                StudentList =  db.rpt_GetExamStudentSubjectClashReport(Convert.ToInt32(ddlExam.SelectedValue)).ToList();
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reporting/Reports/rpt_GetExamStudentSubjectClashReport.rdlc");
                ...
                {
                    OpenReportInPDF("rpt_GetExamStudentSubjectClashReport.pdf");
                }
            }
            catch (Exception)
            {
        ...
        protected void LoadReport(string searchOrPrint)
        {
            ...
            switch (reportName)
            {
                case "rpt_GetExamStudentSubjectClashReport":
                    lblReportTitle.InnerHtml = "Exam Student Subject Clash Report";
