...
{
    public class ExamDutyRosterAttandanceForAdminController : Controller
    {
        private ModelCMSNewContainer db = new ModelCMSNewContainer();
        ExamDutyRosterAttandanceViewModel model = new ExamDutyRosterAttandanceViewModel();
        // GET: ExamDutyRosterAttandanceForAdmin
        [HttpGet]
        public ActionResult Index(string ExamID, string ExamDateID, string ExamDateTimeSlotID, string BatchID, string BatchProgramID, string YearSemesterNo, string ProgramCourseID,string RoomID, string message)
        {
            ...
            ViewBag.RoomID = new SelectList(RoomList, "RoomID", "RoomName", RoomID);
            ...
            model.RosterList = db.GetExamDutyRosterAttandanceForAdmin(Convert.ToInt32(ExamID), Convert.ToInt32(ExamDateID), Convert.ToInt32(ExamDateTimeSlotID), Convert.ToInt32(BatchID), Convert.ToInt32(BatchProgramID), Convert.ToInt32(YearSemesterNo), Convert.ToInt32(ProgramCourseID), Convert.ToInt32(RoomID)).ToList();
            return View(model);
        }


        [HttpPost]
        public ActionResult Index(int ExamID, int ExamDateID, int ExamDateTimeSlotID, int BatchID, int BatchProgramID, int YearSemesterNo, int ProgramCourseID, int RoomID)
        {
            List<Room> RoomList = new List<Room>();
            List<Exam> ExamList = new List<Exam>();
            ExamList = db.Exams.ToList();
            ...
            ViewBag.MessageType = "";
            ViewBag.Message = "";
            model.RosterList = db.GetExamDutyRosterAttandanceForAdmin(ExamID, ExamDateID, ExamDateTimeSlotID, BatchID, BatchProgramID, YearSemesterNo, ProgramCourseID, RoomID).ToList();
        ...
        [HttpPost]
        public ActionResult SaveRosterAttendance(string ExamID, string ExamDateID, string ExamDateTimeSlotID, string BatchID, string BatchProgramID, string YearSemesterNo, string ProgramCourseID, string RoomID, List<GetExamDutyRosterAttandanceForAdmin_Result> RosterList)
        {
            string message = "1";
            try
            {
                ...
                {
                    db.UpdateExamRosterAttandanceByID(RosterList[i].ExamRosterDetailID, RosterList[i].StudentPresentStatusID, RosterList[i].Remarks, RosterList[i].ArivalTime, RosterList[i].ArivalDate, CurrentUserID);
                    if (message == "0")
                    {
                        break;
                    }
            ...
            return RedirectToAction("Index", new { ExamID = ExamID, ExamDateID = ExamDateID, ExamDateTimeSlotID = ExamDateTimeSlotID, BatchID = BatchID, BatchProgramID = BatchProgramID, YearSemesterNo = YearSemesterNo, ProgramCourseID = ProgramCourseID, RoomID = RoomID, message = message });
        ...
        private List<Room> LoadRoomList() {
            ...
            RoomObj.RoomID = 0;
            RoomObj.RoomName = "--All--";
            RoomList = db.Rooms.ToList();
            RoomList.Insert(0,RoomObj);
            return RoomList;
