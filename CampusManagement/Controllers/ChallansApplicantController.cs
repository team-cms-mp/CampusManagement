...
{
    [Authorize]
    public class ChallansApplicantController : Controller
    {
        ...
        {
            model.Challans = db.GetAllChallans(0, "", 0, "Applicant").ToList();
        ...
        public ActionResult Create(Challan challan, FormCollection fc)
        {
                ...
                if (string.IsNullOrEmpty(ErrorMessage))
                {
                    ...
                    if (Data != "<DATA></DATA>")
                    {
                        db.Post_FinancialImpact_Challan(Data, Convert.ToInt32(Session["emp_id"])
                            , Convert.ToInt32(Session["SubDeptid"]), Convert.ToInt32(Session["dept_id"]), "Applicant");
                    }
            ...
            model.Challans = db.GetAllChallans(challan.BatchProgramID, challan.FormNo, challan.YearSemesterNo, "Applicant").ToList();
            model.SelectedChallan = null;
            model.DisplayMode = "WriteOnly";
        ...
        public JsonResult ApproveStudentChallan(int? ChallanID, string depositDate)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(ErrorMessage))
                {
                    db.ApproveStudentChallan(Convert.ToDateTime(depositDate), ChallanID, EmpID, "Applicant");
        ...
        {
            GetEnrolledStudent_Result objES = db.GetEnrolledStudent(FormNo, "Applicant").FirstOrDefault();
            ViewBag.objES = objES;
            ViewBag.AccountID = new SelectList(db.Finance_Bank_Accounts, "Account_ID", "Account_No");
            ViewBag.YearSemesterNo = new SelectList(db.Semesters, "YearSemesterNo", "YearSemesterNo");
            ViewBag.MessageType = "";
