...
{
    [Authorize]
    public class ApplicantDiscountsController : Controller
    {
        private ModelCMSNewContainer db = new ModelCMSNewContainer();
        SF_GetStudentApplicantDiscounts_ResultViewModel model = new SF_GetStudentApplicantDiscounts_ResultViewModel();

        [HttpGet]
        public ActionResult ApplicantsForDiscount()
        {
            ViewBag.MessageType = "";
            ViewBag.Message = "";
            List<SF_GetApplicantsForDiscounts_Result> lstStudents = db.SF_GetApplicantsForDiscounts("", 0, 7).ToList();
            return View(lstStudents);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ApplicantsForDiscount(string paramSearch, int? BatchID, int? BatchProgramID, int? StatusID)
        {
            if (BatchID == null)
            {
                BatchID = 0;
            ...
            List<SF_GetApplicantsForDiscounts_Result> lstStudents = db.SF_GetApplicantsForDiscounts(paramSearch, BatchProgramID, StatusID).ToList();
            return View(lstStudents);
        }

        public ActionResult Index(int? StudentID)
        {
            model.SF_GetStudentApplicantDiscounts_Results = db.SF_GetStudentApplicantDiscounts(StudentID, 0).ToList();
        ...
        public ActionResult Create(SF_StudentDiscount discount)
        {
            ...
            try
            {
                SF_StudentDiscount dis = db.SF_StudentDiscount.FirstOrDefault(d => d.ApplicantID == discount.StudentID && d.YearSemesterNo == 1 && d.DiscountTypeID == discount.DiscountTypeID);
                if (dis != null)
                {
                    count++;
            ...
            model.SF_GetStudentApplicantDiscounts_Results = db.SF_GetStudentApplicantDiscounts(discount.StudentID, 0).ToList();
            model.SelectedSF_GetStudentApplicantDiscounts_Result = null;
            model.DisplayMode = "WriteOnly";
            ViewBag.DiscountTypeID = new SelectList(db.SF_StudentDiscountType, "DiscountTypeID", "DiscountTypeName");
            ViewBag.MessageType = "";
        ...
        public ActionResult ApproveUnapprove(int? id, string type, int? StudentID)
        {
            try
            {
                Applicant stu = db.Applicants.FirstOrDefault(s => s.AddAppID == StudentID);
                int EmpID = Convert.ToInt32(Session["emp_id"]);
                if (type == "Approved")
                {
                    db.SF_Insert_StudentService(0, stu.FormNo, StudentID, stu.BatchProgramID, 1, EmpID, 9, id, 0);
                }
                else if (type == "Unapproved")
                {
                    db.SF_Insert_StudentService(0, "", StudentID, stu.BatchProgramID, 1, EmpID, 7, id, 0);
                }
            }
            ...
            return RedirectToAction("Index", "ApplicantDiscounts", new { StudentID = StudentID });
        }
