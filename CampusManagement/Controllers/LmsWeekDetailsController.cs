...
{
    public class LmsWeekDetailsController: Controller
    {
        int count = 0;
        string ErrorMessage = "";
        ...
        public ActionResult Index(int? LMSWeekID)
        {
            ...
            model.LMSWeekDetails = db.LMSWeekDetails.Where(x => x.LMSWeekID == LMSWeekID && x.CreatedBy  == EmpTeacherID).OrderByDescending(a => a.LMSWeekDetailID).ToList();
        ...
        public ActionResult Create(LMSWeekDetail LMS)
        {
            try
            {
                LMSWeekDetail d = db.LMSWeekDetails.FirstOrDefault(de => de.Title == LMS.Title && de.LMSWeekID == LMS.LMSWeekID);
                if (d == null)
                {
                    try
                    {
